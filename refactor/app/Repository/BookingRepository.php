...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        if (empty($cuser)) {
            return response()->json([
                'error' => true,
                'error_message' => "Sorry, the user is not exist!"
            ]);
        }
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type != config('app.role_ids.customer_role_id')) {
            return [
                'status' => 'fail',
        ...
        if (in_array('normal', $data['job_for']) && in_array('certified', $data['job_for'])) {
            ...
            $dataCertified = 'n_health';
        }

        $data['certified'] = $dataCertified;
    ...
    public function storeJobEmail($data)
    {
        ...
        $job->reference = !empty($data['reference']) ? $data['reference'] : '';
        $user = $job->user()->first();

        $job->address = !empty($data['address']) ? $data['adderss'] : $user->userMeta->address;
        ...
        $this->mailer->send($email, $userName, $subject, 'emails.job-created', $send_data);
    ...
    public function sendNotificationTranslator($job, $data = [], $exclude_user_id)
    {
        ...
        // This will run very slowly when the users data grow a lot
        // This code can be optimized better to get the users for this job only but i don't have the exact information of the job and translator relation
     ...
     * @param $data New data for the job
     * @param $cuser User editing the job
     * @return mixed
     */
    public function updateJob($id, $data, $cuser)
    {
        ...
        if (empty($job)) {
            return [
                'status' => 'fail',
                'message' => "Sorry, the job is no longer exist!"
            ];
        }
    ...
    public function acceptJobWithId($job_id, $cuser)
    {
        ...
        if ($job->status == 'pending' && Job::insertTranslatorJobRel($cuser->id, $job_id)) {
            ...
            if ($this->isNeedToSendPush($jobUser->id)) {
                $users_array = array($jobUser);
                $this->sendPushNotificationToSpecificUsers($users_array, $job_id, $data, $msg_text, $this->isNeedToDelayPush($jobUser->id));
            }
    ...
    public function reopen($request)
    {
        ...
        if (empty($job)) {
            return [
                'error' => true,
                'error_message' => "Sorry, the job is not exist"
            ];
        }
